...
import org.komapper.sql.SqlLocation as Loc
    ...
    sealed class Token : SqlNode() {
        ...
        data class Space(override val token: String) : Token() {
            companion object {
                ...
                fun of(token: String): Space = map.getOrElse(token) { Space(token) }
            }
        }

        data class Other(override val token: String) : Token() {
            companion object {
                private val map = listOf(",", "=", ">", "<", "-", "+", "*", "/", "(", ")", ";").associateWith(::Other)
                fun of(token: String): Other = map.getOrElse(token) { Other(token) }
            }
        }
    }
}
