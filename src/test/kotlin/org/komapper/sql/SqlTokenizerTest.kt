...
import org.komapper.sql.SqlTokenType.*
...
class SqlTokenizerTest {
    ...
    fun testOption() {
        ...
        assertEquals(WHITESPACE, tokenizer.next())
        assertEquals(" ", tokenizer.token)
        assertEquals(OPEN_BRACKET, tokenizer.next())
        assertEquals("(", tokenizer.token)
        assertEquals(EOF, tokenizer.next())
        assertEquals("", tokenizer.token)
    }
    ...
    @Test
    fun testExpandBlockComment() {
        val tokenizer = SqlTokenizer("select /*%expand*/* from")
        assertEquals(SELECT, tokenizer.next())
        assertEquals("select", tokenizer.token)
        assertEquals(WHITESPACE, tokenizer.next())
        assertEquals(" ", tokenizer.token)
        assertEquals(EXPAND_DIRECTIVE, tokenizer.next())
        assertEquals("/*%expand*/", tokenizer.token)
    ...
    @Test
    fun testExpandBlockComment_alias() {
        val tokenizer = SqlTokenizer("select /*%expand e*/* from")
        assertEquals(SELECT, tokenizer.next())
        assertEquals("select", tokenizer.token)
        assertEquals(WHITESPACE, tokenizer.next())
        assertEquals(" ", tokenizer.token)
        assertEquals(EXPAND_DIRECTIVE, tokenizer.next())
        assertEquals("/*%expand e*/", tokenizer.token)
        assertEquals(OTHER, tokenizer.next())
        assertEquals("*", tokenizer.token)
        assertEquals(WHITESPACE, tokenizer.next())
        assertEquals(" ", tokenizer.token)
